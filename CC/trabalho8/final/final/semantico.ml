...
let rec verifica_cmd amb tiporet cmd =
  ...
  | For (var, inicio, fim, avanco, corpo) ->
    ...
    let _ = mesmo_tipo (posicao fim)
            "O comando ATE deveria ser do tipo %s e nao %s"
            TipoInt tinff in
    let _ = mesmo_tipo (posicao avanco)
            "O comando PASSO deveria ser do tipo %s e nao %s"
            TipoInt tinfa in

    let corpo_tipo = List.map (verifica_cmd amb tiporet) corpo in
    For (var_tipo,inicio_tipo,fim_tipo,avanco_tipo,corpo_tipo) 
