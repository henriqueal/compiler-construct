...
let obtem_nome_tipo_var exp = let open T in
    match exp with
      | ExpVar (v,tipo) ->
          (match v with
             | Ast.VarSimples (nome,_) -> (nome,tipo)
          )
...
let rec interpreta_exp amb exp =
  ...
  match exp with
	...
	| ExpOpUn ((op, top), (dir, tdir)) ->
	  let vdir = interpreta_exp amb dir in 
	  	 let interpreta_aritmetico () =
	       ( match tdir with
	           | TipoInt ->
	              (match op with
		             | Positivo ->  ExpInt (~+(pega_int vdir), top)
		             | Negativo -> ExpInt (~-(pega_int vdir), top)		            
		             | _ -> failwith "interpreta_aritmetico_unario"
		          )
	           | TipoFloat ->
	              (match op with
		             | Positivo ->  ExpFloat (~+.(pega_float vdir), top)
		             | Negativo -> ExpFloat (~-.(pega_float vdir), top)		            
		             | _ -> failwith "interpreta_aritmetico_unario"
		          )
		       | _ -> failwith "interpreta_logico_unario" 
	       )
	     and interpreta_logico () =	        
      	   ( match tdir with
      	   	   | TipoInt ->
      	   	      (match op with
               		| Nao -> ExpInt (converte_bool (not (volta_bool (pega_int vdir))), top)
               		| _ -> failwith "interpreta_logico_unario"
               	  )	
               | _ -> failwith "interpreta_logico_unario"  
           )
         ...
         let valor = ( match (classificaUn op) with
		            ...
		            | Relacional -> let msg = "Warning" in failwith msg
          )
         in
         valor

    | ExpOpBin ((op, top), (esq, tesq), (dir, tdir)) ->
      ...
      and  vdir = interpreta_exp amb dir in
	    let interpreta_aritmetico () =
	      ( match tesq with
		      ...
		      | TipoChar ->
		          (match op with
		             | Soma ->  ExpChar (char_of_int (int_of_char (pega_char vesq) + int_of_char (pega_char vdir)), top)
		             | Sub -> ExpChar (char_of_int (int_of_char (pega_char vesq) - int_of_char (pega_char vdir)), top)
       ...
       ( match tesq with
           | TipoInt ->
	         (match op with
	            ...
	            | Dif   -> ExpInt (converte_bool (pega_int vesq != pega_int vdir), top)
	            | MaiorIg   -> ExpInt (converte_bool (pega_int vesq >= pega_int vdir), top)
	            | MenorIg   -> ExpInt (converte_bool (pega_int vesq <= pega_int vdir), top)
           ...
           | TipoFloat ->
	         (match op with
	            ...
	            | Dif   -> ExpInt (converte_bool (pega_float vesq != pega_float vdir), top)
	            | MaiorIg   -> ExpInt (converte_bool (pega_float vesq >= pega_float vdir), top)
	            | MenorIg   -> ExpInt (converte_bool (pega_float vesq <= pega_float vdir), top)
	            | _ -> failwith "interpreta_relacional"
	         )
           | TipoChar ->
	         (match op with
	            ...
	            | Dif   -> ExpInt (converte_bool (pega_char vesq != pega_char vdir), top)
	            | MaiorIg   -> ExpInt (converte_bool (pega_string vesq >= pega_string vdir), top)
	            | MenorIg   -> ExpInt (converte_bool (pega_string vesq <= pega_string vdir), top)
	            | _ -> failwith "interpreta_relacional"
	         )
           | _ ->  failwith "interpreta_relacional"
      )
      ...
      ( match tesq with
          | TipoInt ->
            ( match op with
                ...
                | Elog ->   ExpInt (converte_bool (volta_bool (pega_int vesq) && volta_bool (pega_int vdir)), top)
...
and interpreta_cmd amb cmd =
   ...
   match cmd with
     ...
     leia_var nts
       ...
       in
       let _ = imprima exps in print_string ""
...
let insere_declaracao_var amb dec =
    match dec with
        A.DecVarG (nome, tipo) ->  Amb.insere_local amb (fst nome) tipo None

let insere_declaracao amb dec =
  let open A in
    match dec with
      A.Funcao {nome; tipReturn; args; locais; cmds} ->
        let nome = fst nome in
        let formais = List.map (fun (n,t) -> ((fst n), t)) args in
        Amb.insere_fun amb nome formais locais tipReturn cmds
