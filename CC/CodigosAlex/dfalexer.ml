...
let obtem_token_e_estado str el =
  ...
  let proximo_el = { el with pos_inicial = fim + 1;
                             ...
                             dfa = { el.dfa with estado = estado_inicial;
                                                 posicao = fim + 1}}
  in
  let _ = Printf.printf "inicio = %d fim = %d tamanho = %d\n\n" inicio fim tamanho
  in
   (token, proximo_el)
...
let rec analisador (str:entrada) tam el =
  ...
  then if el.pos_final >= 0
       then let token, proximo_el = obtem_token_e_estado str el in
            [ token; EOF ]
       else [ EOF ]
  else if estado_atual = estado_morto
       then let token, proximo_el = obtem_token_e_estado str el in
            let _ = Printf.printf "Morto\n"  in
            let _ = Printf.printf "pos:%d estado:%d tam:%d\n" posicao_atual estado_atual proximo_el.dfa.posicao in
            token :: analisador str tam proximo_el
       else 
         let simbolo = str.[posicao_atual]
         and transicao = el.dfa.transicao
         and estado_atual = el.dfa.estado in
         let proximo_estado = transicao estado_atual simbolo in
         let _ = Printf.printf "simbolo:%c pos:%d estado:%d proximo:%d\n" simbolo posicao_atual estado_atual proximo_estado in
