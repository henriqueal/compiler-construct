...
%token <float * Lexing.position> PFLOAT
...
%token <Lexing.position> INTEIRO CADEIA BOOLEANO FLOAT
...
declaracao_de_variavel:
  ids = separated_nonempty_list(VIRG, ID) DPTOS t = tipo  {
...
tipo_registro: REGISTRO
                 campos=nonempty_list(id=ID DPTOS tp=tipo  { (id,tp) } )
...
comando_entrada:  xs=separated_nonempty_list(VIRG, expressao) ENTRADA {
                   CmdEntrada xs
               }
...
comando_enquanto:
    | WHILE APAR e=expressao FPAR DO stm=comando* FIM { While (e,stm) }
    ;

comando_para:
    | FOR lv=expressao VIRG e1=expressao VIRG e2=expressao p=passo DO stm=comando* FIM { For(lv, e1, e2, p, stm)  }
    ;
